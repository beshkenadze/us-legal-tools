  ...
  try {
    ...
    if (!swaggerData) {
      ...
      try {
        ...
        const jsonMatch = content.match(/<(pre|code)[^>]*>({[\s\S]*?"swagger"[\s\S]*?})<\/(pre|code)>/);
        if (jsonMatch) {
    ...
    console.error('\n❌ Browser download failed:', error instanceof Error ? error.message : String(error));
    return false;
  } finally {
    // Clean up resources
    if (page) {
...
async function main() {
  ...
  if (!success) {
    ...
    console.error('   docker run -d --name chrome -p 9222:9222 --shm-size="2g" browserless/chrome:latest');
